...
class AggregatedJaccardIndex(AsymmetricMeasureMixin, Measure):
    r"""
    Defines the Aggregated Jaccard Index (AJI) as described in Kumar et al.
    (2017).

    The AJI is computed by finding optimal object matches between ground truth
    and predicted objects, then computing the aggregated intersection and union
    across all matched pairs and unmatched objects.

    The AJI is defined as:

    .. math:: \mathrm{AJI} = \frac{\sum_k \left|G_k \cap P_{\sigma(k)}\right|}
        {\sum_k \left|G_k \cup P_{\sigma(k)}\right| +
        \sum_{j \notin \sigma} \left|P_j\right|}

    where :math:`G_k` are ground truth objects, :math:`P_j` are predicted
    objects, :math:`\sigma(k)` represents the optimal matching function that
    maps each ground truth object to at most one predicted object based on
    maximum IoU, and the second sum includes all unmatched predicted objects.
    ...
    def __init__(self, min_ref_size: int = 1, iou_threshold: float = 0.5,
                 **kwargs) -> None:
        super().__init__(**kwargs)
        assert min_ref_size >= 1, 'min_ref_size must be 1 or larger'
    ...
    def compute(self, actual: LabelImage) -> List[float]:
        ...
        # For efficiency, we'll use a greedy approach that matches each GT
        # object to the predicted object with highest IoU above threshold
        ...
        for ref_label in ref_labels:
            ...
            # Check all candidate predicted objects that overlap with this GT
            # object
